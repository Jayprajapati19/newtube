    ...
    getLiked: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const data = await db
                ...
                .from(videos)
                .innerJoin(users, eq(videos.userId, users.id))
                .innerJoin(viewerVideoReactions, eq(videos.id, viewerVideoReactions.videoId)) // <-- join the CTE here
                .where(and(
                    eq(videos.visibility, "public"),
                    cursor
                        ? or(
                            // Use the joined CTE for likedAt
                            // viewerVideoReactions.likedAt is now available
